...
public class AlgumaParser {
    ...
    private final static int TAMANHO_BUFFER = 10; // tamanho do buffer
    List<Token> bufferTokens; // lista de tokens
    AlgumaLexico lex; // referencia para o lexico
    boolean chegouNoFim = false; // para saber quando terminou a leitura

    public AlgumaParser(AlgumaLexico lex) {
        this.lex = lex;
        bufferTokens = new ArrayList<Token>();
        lerToken(); // inicio da analise sintatica
    }

    private void lerToken() { // vai buscar o lexico e preencher o buffer
        if (bufferTokens.size() > 0) {
            bufferTokens.remove(0);
        }
        while (bufferTokens.size() < TAMANHO_BUFFER && !chegouNoFim) { //carregar o buffer
            Token proximo = lex.proximoToken();
            bufferTokens.add(proximo);
            if (proximo.nome == TipoToken.Fim) { // se for o final do arquivo
                chegouNoFim = true;
            }
        }
        System.out.println("Lido:  " + lookahead(1)); // impressão para saber se foi lido
    }
    ...
    Token lookahead(int k) {
        if (bufferTokens.isEmpty()) { // se estiver vazio
            return null;
        }
        if (k - 1 >= bufferTokens.size()) {
            return bufferTokens.get(bufferTokens.size() - 1); // pega o ultimo
        }
